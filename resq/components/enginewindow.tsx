...
export default function EngineWindow() {
  const router = useRouter();

  // CONSTANTS
  const SERVICE_NAME = "Engine Repair";
  const PRICE = 299;
  const IMAGE_URL = "https://www.tinkerdiy.com/hubfs/Car%20Engine.jpg";
  ...
  const handleRequest = async () => {
    try {
      ...
      alert("Engine inspection service requested successfully!");
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.infoCard}>
          ...
          <Text style={styles.description}>
            Engine problems can arise from overheating, oil leakage, spark plug failure,
            or internal wear. A complete inspection is required for proper diagnosis.
          </Text>
        </View>

        {/* INSPECTION ALERT */}
        <View style={styles.alertBox}>
          <Ionicons name="warning-outline" size={28} color="#FFC94A" />

          <View style={{ marginLeft: 12 }}>
            <Text style={styles.alertTitle}>Inspection Required</Text>
            <Text style={styles.alertText}>• Mechanic will inspect engine components.</Text>
            <Text style={styles.alertText}>• Inspection fee applies.</Text>
            <Text style={styles.alertText}>• Final cost depends on the nature of engine damage.</Text>
          </View>
        </View>
